...
export const AuditEngine = {
	...
	async auditTest(
		...
		console.log(
			`📋 [AUDIT] Instructions parsées pour ${testId}:`,
			instructions
		);
		...
		for (let i = 0; i < instructions.length; i++) {
			...
			console.log(
				`⚙️ [AUDIT] Exécution instruction ${i + 1}/${instructions.length} pour ${testId}:`,
				instruction
			);

			const result = await AuditEngine.executeInstruction(
				testId,
				instruction,
				testTitle
			);
			console.log(
				`📊 [AUDIT] Résultat instruction ${i + 1} pour ${testId}:`,
				result
			);
		...
		const overallStatus = AuditEngine.determineOverallStatus(results);
		console.log(
			`✅ [AUDIT] Audit terminé pour ${testId} - Statut global: ${overallStatus}`
		);
	...
	async executeInstruction(
		...
		console.log(
			`🔧 [AUDIT] Exécution instruction pour ${testId} - Type: ${instruction.checkType}`,
			instruction
		);
		...
		try {
			...
			switch (instruction.checkType) {
				case 'presence':
					...
					result = AuditEngine.checkPresence(testId, instruction, testTitle);
					break;
				case 'attribute':
					console.log(`🏷️ [AUDIT] Vérification attributs pour ${testId}`);
					result = AuditEngine.checkAttributes(testId, instruction, testTitle);
					break;
				case 'content':
					console.log(`📝 [AUDIT] Vérification contenu pour ${testId}`);
					result = AuditEngine.checkContent(testId, instruction, testTitle);
					break;
				case 'structure':
					console.log(`🏗️ [AUDIT] Vérification structure pour ${testId}`);
					result = AuditEngine.checkStructure(testId, instruction, testTitle);
					break;
				default:
					console.log(
						`⚠️ [AUDIT] Type de vérification non supporté pour ${testId}: ${instruction.checkType}`
					);
					result = {
						testId,
						status: 'NA',
						message: `Test ${testId}: ${AuditEngine.getTestContext(testTitle)} - Vérification manuelle requise`
					};
			}
			...
			console.error(
				`❌ [AUDIT] Erreur lors de l'exécution pour ${testId}:`,
				error
			);
			return {
				testId,
				status: 'NA',
				message: `Test ${testId}: ${AuditEngine.getTestContext(testTitle)} - Erreur lors de l'exécution: ${error instanceof Error ? error.message : 'Erreur inconnue'}`
			};
		}
	},
	 ...
	 */
	getTestContext(testTitle: string): string {
		...
		return mainPart.length > 100
			? `${mainPart.substring(0, 100)}...`
			: mainPart;
	},
	...
	checkPresence(
		...
		if (!instruction.selector) {
			return {
				testId,
				status: 'NA',
				message: `Test ${testId}: ${AuditEngine.getTestContext(testTitle)} - Impossible de déterminer les éléments à vérifier`
			};
		}
		...
		if (elements.length === 0) {
			return {
				testId,
				status: 'NA',
				message: `Test ${testId}: ${AuditEngine.getTestContext(testTitle)} - Aucun élément HTML correspondant trouvé sur la page`,
				details: `Recherche: ${instruction.selector}`
			};
		}

		const elementTypes = Array.from(
			new Set(Array.from(elements).map((el) => el.tagName.toLowerCase()))
		);

		return {
			testId,
			status: 'OK',
			message: `Test ${testId}: ${AuditEngine.getTestContext(testTitle)} - ✅ ${elements.length} élément(s) trouvé(s)`,
	...
	checkAttributes(
		...
		for (const element of elements) {
			...
			if (elementHasIssues) {
				elementsWithIssues++;
				results.push(
					`❌ ${element.tagName.toLowerCase()}: ${elementResults.join(', ')}`
				);
			} else {
				results.push(
					`✅ ${element.tagName.toLowerCase()}: ${elementResults.join(', ')}`
		...
		const summary = hasFailures
			? `Test ${testId}: ${AuditEngine.getTestContext(testTitle)} - ❌ ${elementsWithIssues}/${totalElements} élément(s) avec problème(s)`
			: `Test ${testId}: ${AuditEngine.getTestContext(testTitle)} - ✅ ${totalElements} élément(s) conforme(s)`;
	...
	checkContent(
		...
		const summary = hasEmptyContent
			? `Test ${testId}: ${AuditEngine.getTestContext(testTitle)} - ❌ ${elementsWithIssues}/${totalElements} élément(s) avec contenu vide`
			: `Test ${testId}: ${AuditEngine.getTestContext(testTitle)} - ✅ ${totalElements} élément(s) avec contenu valide`;
	...
	checkStructure(
		...
		if (
			instruction.text.toLowerCase().includes('hiérarchie') ||
			instruction.text.toLowerCase().includes('titre')
		) {
			return AuditEngine.checkHeadingHierarchy(testId, testTitle);
		}

		return {
			testId,
			status: 'NA',
			message: `Test ${testId}: ${AuditEngine.getTestContext(testTitle)} - Vérification de structure non implémentée`
		};
	},
	 ...
	 */
	checkHeadingHierarchy(testId: string, testTitle: string): AuditResult {
	 ...
	 */
	determineOverallStatus(results: AuditResult[]): 'OK' | 'FAIL' | 'NA' {
