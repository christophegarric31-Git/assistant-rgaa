...
import {startAudit, setCurrentTest, updateProgress, addTestResult, completeAudit, setAuditError} from './audit';
import {selectAllThemes, selectAllCriteria, selectAllTests} from './reference';
 ...
 */
export const startFullAudit = createAsyncThunk<
	void,
	void,
	{state: RootState}
>(
	...
	async (_, {dispatch, getState}) => {
		...
		try {
			...
			const sortedTests = Object.values(tests).sort((a, b) => a.id.localeCompare(b.id));
			
			for (const test of sortedTests) {
				currentTest++;
				console.log(`🔍 [AUDIT] Test ${currentTest}/${totalTests}: ${test.id} - ${test.title}`);
				
				// Récupérer le critère et le thème associés
				const criterion = criteria[test.criterionId];
				const theme = themes[criterion.themeId];
				
				console.log(`📋 [AUDIT] Critère associé: ${criterion.id} - ${criterion.title}`);
				...
				console.log(`📖 [AUDIT] Instructions trouvées pour ${test.id}:`, instructions ? 'OUI' : 'NON');
				...
				if (instructions) {
					...
					console.log(`✅ [AUDIT] Audit terminé pour ${test.id} - Statut: ${testResult.overallStatus}`);
					results.push(testResult);
					dispatch(addTestResult(testResult));
				} else {
					console.log(`⚠️ [AUDIT] Pas d'instructions pour ${test.id} - Test ignoré`);
				}
			...
			const errorMessage = error instanceof Error ? error.message : 'Erreur inconnue lors de l\'audit';
...
>(
	...
	async (_, {getState}) => {
		...
		const csvHeaders = 'Thème;Critère;Sous critère;Libellé du sous critère;Statut\n';
		const csvRows = auditResult.results.map(result => {
			...
			const cleanThemeTitle = theme?.title?.replace(/<[^>]*>/g, '').trim() || result.themeTitle;
			const cleanCriterionTitle = criterion?.title?.replace(/<[^>]*>/g, '').trim() || result.criterionTitle;
			const cleanTestTitle = result.testTitle?.replace(/<[^>]*>/g, '').trim() || result.testTitle;
		...
		link.setAttribute('download', `audit-rgaa-${new Date().toISOString().split('T')[0]}.csv`);
